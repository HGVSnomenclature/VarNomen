...
*	deletion
	...
	*	**PCR**
		*	size
		:	when a fragment containing a deletion has been amplified but only its size was determined (and not its sequence), the variant should be reported as **g.(position-fragment-start\_position-fragment-end)del(#)** or **g.(position-fragment-start\_position-fragment-end)ins(#)** with "#" indicating the size (or estimated size) of the deletion/insertion in nucleotides. 
		:	Example NM\_000333.3:c.(4\_246)del(15), NM\_000333.3:c.(4\_246)ins(9), [_see Repeated sequences_](/recommendations/DNA/variant/repeated/)
		:	Example NC\_000003.12:g.63912687AGC[(50\_60)], NC\_000003.12:g.63912687AGC[(60\_?)], [_see Alleles_](/recommendations/DNA/variant/alleles/).
		* present/absent
	...
	*	MLPA
	...
	:	_**NOTE**_: in diagnostics it is common practice to describe MLPA results as a change affecting the entire exon. When the exact probe location would be used, variant description is precise but rather complex and difficult to understand. Furthermore, assuming the MLPA probe pair used hybridizes from position c.3211 to c.3266, which location should be used (only part of the target sequence might be deleted)?  To make descriptions not too complicated, when an exon probe is affected, it is therefore **allowed** to assume the entire exon is affected (deleted) and to base the description on the locations of the exon/intron borders (exception; large exons are measured using several probes). LOVD's ["Reading frame checker"](https://databases.lovd.nl/shared/scripts/readingFrameChecker.php?gene=DMD&transcript=NM_004006.2){:target="\_blank"} might be a useful tool assisting you to generate correct HGVS descriptions.
	...
	*	array (CGH or SNP)
	:	since the probe sequences used are known genomic rearrangements detected using arrays (CGH or SNP arrays) can be described using genomic coordinates;  **(position-last-normal\_position_first-altered)\_ (position-last-altered\_position-first-normal)**. Similar, alternative descriptions have been suggested by the [ISCN guidelines](http://www.karger.com/Article/FullText/353118){:target="\_blank"}. Note also that more examples can be found under [_Complex (HGVS/ISCN)_](/recommendations/DNA/variant/complex/), including array-characterised translocations.
...
*	**insertion**
	...
	*	present/absent
	...
	:	The description of the insertion, "ins(last-normal\_first-inserted)\_(last-inserted\_first-normal)", is based on the uncertainty of the extent of the inserted sequence. To describe the inserted sequence, follow the standard recommendations, i.e. try to describe it as precise as possible, including regions of uncertainty.
